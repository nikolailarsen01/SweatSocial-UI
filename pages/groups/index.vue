...
const myInvites = ref([]);
await http.get(`invites/group`).then(function (res){
...
<template>
  ...
  <Modal class="max-w-xl" v-show="showModal">
    ...
    <div class="mb-4">
      ...
      <div class="text-sm mt-1 pt-1 border-t border-gray-700">
        <div class="flex place-items-center gap-2 bg-stone-400 w-fit p-2 pr-4 rounded-full">
          <img :src="baseUrl + groupPreview.group_owner.image" class="rounded-full h-10 w-10">
  ...
  <div class="w-full mb-2">
    <div class="flex justify-between">
      <label class="underline">Mine grupper ({{ myGroups.length }})</label>
    ...
    <div :class="[ allMyGroups ? 'h-64 overflow-y-scroll' : 'h-[5.25rem]' ]" class="bg-stone-200 mt-2 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 rounded-t p-2 gap-2 overflow-hidden transition-all duration-250 ease">
      <div @click="showGroupPreview(group.group)" v-for="group in myGroups" class="h-18 bg-stone-400 hover:bg-stone-500 hover:text-white cursor-pointer col-span-1 rounded p-2">
        <h1 class="truncate"><Icon v-if="group.group.locked" name="heroicons:lock-closed"/> {{ group.group.group_name }}</h1>
        <p class="text-sm mt-1 pt-1 border-t border-gray-700">{{ group.group.members + ' medlem(er)' }}</p>
      </div>
    </div>
  ...
  <div class="w-full mb-2">
    <label class="underline">Gruppe invitationer (5)</label>
    <div :class="[ allInvites ? 'h-64 overflow-y-scroll' : 'h-[5.25rem]' ]" class="bg-stone-200 mt-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 rounded-t p-2 gap-2 overflow-hidden transition-all duration-250 ease">
      <div @click="showGroupPreview(invite.group)" v-for="invite in myInvites" class="h-18 bg-stone-400 hover:bg-stone-500 hover:text-white cursor-pointer col-span-1 rounded p-2">
        <h1 class="truncate"><Icon v-if="invite.group.locked" name="heroicons:lock-closed"/> {{ invite.group.group_name }}</h1>
        <p class="text-sm mt-1 pt-1 border-t border-gray-700">{{ invite.group.members + ' medlem(er)' }}</p>
      </div>
    </div>
    <div @click="allInvites = !allInvites" class="bg-stone-400 rounded-b p-1 cursor-pointer text-center pr-3"><Icon :class="{ 'rotate-180' : allInvites }" name="heroicons:chevron-down-solid"/></div>
  </div>
